...
// HELPER: The card component with the corrected shiny overlay
const AuctionCard = ({ auction, isActive }: { auction: Auction, isActive: boolean }) => {
  return (
    // Wrapper for the animated gradient border effect
    <div className="relative h-full w-full rounded-[2.5rem] p-[2px] overflow-hidden">
        {/* The actual gradient that creates the border */}
        <div className={
            `absolute inset-0 rounded-[2.5rem] bg-gradient-to-br from-orange-500/80 via-purple-600/50 to-blue-500/80 transition-opacity duration-500 ${isActive ? 'opacity-100' : 'opacity-0'}`
        } />
        <div className="relative h-full w-full rounded-[calc(2.5rem-2px)] bg-black/70 shadow-[0_12px_48px_0_rgba(255,140,0,0.10),0_8px_32px_0_rgba(31,38,135,0.25)] border border-white/10 overflow-hidden" style={{backdropFilter:'blur(24px)'}}>
            {/* Subtle dark gradient overlay for better text contrast */}
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent" />
            
            {/* FIXED: Shiny glass overlay applied to the ENTIRE card */}
            ...
            <div className="relative z-10 flex flex-col h-full px-5 sm:px-6 pt-5 sm:pt-6 pb-4 sm:pb-5">
                ...
                <div className="flex-shrink-0">
                    ...
                    <div className="flex items-center gap-2 mt-2 flex-wrap">
                        ...
                        <span className={`text-white text-[10px] sm:text-xs px-2.5 py-1 rounded-full font-bold uppercase ${auction.condition === 'new' ? 'bg-green-600/80' : 'bg-gray-500/80'}`}>{auction.condition}</span>
                    </div>
                </div>
                ...
                <div className="relative flex-grow flex items-center justify-center w-full my-3">
                    {auction.images?.[0] ? (
                        <>
                            {/* Glow effect */}
                            <div className={`absolute inset-4 rounded-2xl bg-gradient-to-br from-orange-500/50 via-purple-700/30 to-blue-600/30 blur-3xl transition-opacity duration-500 ${isActive ? 'opacity-100' : 'opacity-0'}`} />
                            <Image
                ...
                {/* Bottom section: Prices, Stats, and CTA */}
                <div className="flex-shrink-0">
                    ...
                    <div className="flex justify-center mt-2">
                        <Button size="lg" className="w-full rounded-full px-6 py-3 text-sm sm:text-base font-semibold bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white shadow-lg transition-all duration-300 transform hover:scale-105">
                            {auction.status === "live" ? "Place a Bid" : auction.status === "upcoming" ? "Get Notified" : "View Auction"}
                        </Button>
                    </div>
                </div>
            </div>
...
function AuctionCardDeck({ auctions }: { auctions: Auction[] }) {
  ...
  function getCardProps(idx: number) {
    ...
    let x = 0, scale = 1, zIndex = 10, rotateY = 0, opacity = 1, filter = "";
    ...
    <div className="relative w-full flex flex-col items-center">
      ...
      <div className="flex justify-center gap-2 mt-8">
        {auctions.map((_, idx) => (
          <button
            key={idx}
            className={`h-2 rounded-full transition-all duration-500 ease-in-out ${idx === activeIndex ? "w-8 bg-gradient-to-r from-orange-500 to-orange-600" : "w-2 bg-white/20 hover:bg-white/40"}`}
